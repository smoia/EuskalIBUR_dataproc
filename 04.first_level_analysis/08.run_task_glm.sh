...
1d_tool.py -infile ${fdir}/${flpr}_echo-1_bold_rej_ort.1D -demean \
		   -write ${tmp}/${flpr}_echo-1_bold_rej_ort.1D -overwrite
...
run3dDeconvolve="${run3dDeconvolve} -ortvec ${tmp}/${flpr}_echo-1_bold_rej_ort.1D meica_rej_ort"
...
case ${task} in
	motor )
		...
		# Plus all trial types against sham 
