	...
	do
		...
		do
			...
			do
				...
				fsleyes render -of CoV_${sub}_${map}_${ftype}_noclip --size 1400 500 --scene lightbox --displaySpace reg/MNI_T1_brain.nii.gz \
						--zaxis 2 --sliceSpacing 21.4 --zrange 15.55 178.75 --ncols 6 --nrows 1 --hideCursor --showColourBar \
						--colourBarLocation right --colourBarLabelSide bottom-right --colourBarSize 70.0 --labelSize 18 \
						--performance 3 reg/MNI_T1_brain.nii.gz --name "MNI_T1_brain" --overlayType volume --alpha 100.0 \
						--brightness 50.0 --contrast 50.0 --cmap greyscale --displayRange 0.0 8337.0 --clippingRange 0.0 8420.37 \
				...
				convert CoV_${sub}_${map}_${ftype}_noclip.png -filter Point -resize 1035x370 +repage -crop 920x212+15+70 +repage ${tmp}/tmp.07pcm/CoV_${sub}_${map}_${ftype}_noclip_res.png

				appending="${appending} ${tmp}/tmp.07pcm/CoV_${sub}_${map}_${ftype}_res.png"
				appending_noclip="${appending_noclip} ${tmp}/tmp.07pcm/CoV_${sub}_${map}_${ftype}_noclip_res.png"
			done

			appending="${appending} +repage ${tmp}/tmp.07pcm/CoV_${sub}_${map}_app.png"
			appending_noclip="${appending_noclip} +repage ${tmp}/tmp.07pcm/CoV_${sub}_${map}_noclip_app.png"
			${appending}
			${appending_noclip}
		done
		...
		composite -geometry +990+0 ${tmp}/tmp.07pcm/CoV_${sub}_lag_${mtype}_noclip_app.png ${tmp}/tmp.07pcm/CoV_${sub}_cvr_${mtype}.png +repage CoV_${sub}_${mtype}.png
	done
done
