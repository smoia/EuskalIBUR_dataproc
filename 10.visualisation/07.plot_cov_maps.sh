		...
		do
			...
			do
				...
				fsleyes render -of CoV_${covtype}_${map}_${ftype} --size 1400 500 --scene lightbox --displaySpace reg/MNI_T1_brain.nii.gz \
						...
						--numInnerSteps 10 --volume 0 CoV_${covtype}_${map}_${ftype}.nii.gz --name "CoV_${map}" --overlayType volume \
				...
				convert CoV_${covtype}_${map}_${ftype}.png -filter Point -resize 1035x370 +repage -crop 920x212+15+70 +repage ${tmp}/tmp.07pcm/CoV_${covtype}_${map}_${ftype}_res.png

				appending="${appending} ${tmp}/tmp.07pcm/CoV_${covtype}_${map}_${ftype}_res.png"
			done

			appending="${appending} +repage ${tmp}/tmp.07pcm/CoV_${covtype}_${map}_app.png"
			${appending}
		done
		composite ${tmp}/tmp.07pcm/CoV_${covtype}_cvr_${mtype}_app.png ${scriptdir}/10.visualisation/canvas/CoV_canvas.png +repage ${tmp}/tmp.07pcm/CoV_${covtype}_cvr_${mtype}.png
		composite -geometry +990+0 ${tmp}/tmp.07pcm/CoV_${covtype}_lag_${mtype}_app.png ${tmp}/tmp.07pcm/CoV_${covtype}_cvr_${mtype}.png +repage CoV_${covtype}_${mtype}.png
	done
done
