...
for k in data.keys():
    for ses in range(0, LAST_SES):
        data[k][f'{ses:02d}'] = pd.DataFrame(columns=[k, 'tissue', 'ftype'])

        for ftype in FTYPE_LIST:
            # Load maps
            if k == 'Lag':
                img = nib.load(f'CVR/sub-{sub}_ses-{ses}_{ftype}_map_cvr/'
                               f'sub-{sub}_ses-{ses}_{ftype}_cvr_lag_masked.nii.gz')
            else:
                img = nib.load(f'CVR/sub-{sub}_ses-{ses}_{ftype}_map_cvr/'
                               f'sub-{sub}_ses-{ses}_{ftype}_cvr_masked.nii.gz')
            ...
            for dk in d.keys():
                ...
                df = pd.DataFrame({k: d[dk], 'tissue': [dk]*d[dk].size,
                                  'ftype': [ftype]*d[dk].size})
                data[k][f'{ses:02d}'] = pd.concat([data[k][f'{ses:02d}'], df],
                                                  ignore_index=True)
    ...
    for n, ftype in FTYPE_LIST:
        ...
        sns.violinplot(data=data[k][f'{ses:02d}'], x=k, y='model',
