...
# 3dTproject -polort 3 -input ${fdir}/${func}.nii.gz -mask ${fdir}/${mask}.nii.gz -prefix ${flpr}_pp.nii.gz -overwrite
...
# 		3dDeconvolve -input ${flpr}_SPC.nii.gz -jobs 2 \
# 		-float -num_stimts 1 \
...
# fslmaths ${flpr}_cvr_idx -mul 0 ${flpr}_spc_over_V
# fslmaths ${flpr}_cvr_idx -mul 0 ${flpr}_tmap
...
fslmaths ${flpr}_cvr_idx -mul ${step} -thr 5 -uthr 705 -bin ${flpr}_cvr_idx_physio_constrained

fslmaths ${flpr}_cvr_idx_physio_constrained -mas ${flpr}_tmap_abs_mask -bin ${flpr}_cvr_idx_mask
fslmaths ../${flpr}_mask -sub ${flpr}_cvr_idx_mask ${flpr}_cvr_idx_bad_vxs

fslmaths ${flpr}_cvr_idx -sub 36 -mas ${flpr}_cvr_idx_mask -add 36 -mas ../${flpr}_mask ${flpr}_cvr_idx_corrected
fslmaths ${flpr}_cvr_idx_corrected -mul ${step} -sub ${poslag} -mul 0.025 ${flpr}_cvr_lag_corrected

fslmaths ${flpr}_cvr -mas ${flpr}_cvr_idx_mask ${flpr}_cvr_masked

fslmaths ${flpr}_cvr_idx_bad_vxs -mul ../tmp.${flpr}_res/${flpr}_betas_0360 -div 71.2 -mul 100 -add ${flpr}_cvr_masked ${flpr}_cvr_corrected
