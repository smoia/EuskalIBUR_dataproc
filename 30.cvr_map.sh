...
do
	...
	do
		...
		# -bucket ${sub}_filt_${vol}_res_deconv/stats_REML_${i}.nii.gz

		# -ortvec ${fmat}_mcf_demean.par motdemean \
		# -ortvec ${fmat}_mcf_deriv1.par motderiv1 \
		
		3dbucket -prefix ${sub}_filt_${vol}_res_deconv/${sub}_filt_${vol}_${i}.nii.gz -abuc ${sub}_filt_${vol}_res_deconv/stats_REML_${i}.nii.gz'[1]' -overwrite
